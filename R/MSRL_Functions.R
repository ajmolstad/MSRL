...
AccPG <- function(y, x, beta, lam1, weight, tol, max.iter = 1e4, quiet = FALSE) {
    ...
    while(outeriter) {
        ...
        if (k.iter > 5) {
            ...
            if ((obj[k.iter - 3] - obj[k.iter])/abs(h.orig) < tol) {
                if (!quiet) {
                    cat("# ------------------------------ ", "\n")
                }
                outeriter <- FALSE
...
NN_ADMM <- function (Y, X, Gamma, Omega, beta, lambda, weight, 
    ...
    while(iterating) {
        ...
        # beta update 
        # ---------------------------------------
        H <- crossprod(X, Y + rho^(-1)*Gammakm1 - Omegakm1 - Xbetakm1)
        ...
        # Gamma udpate
        # -------------------------------------
        Gammak <- Gammakm1 + (1.5)*rho*(Y - Xbetakm1 - Omegak)

        r1 <- sqrt(sum((Y - Xbetakm1 - Omegak)^2))
        s1 <- sqrt(sum(crossprod(rho*X, Omegak - Omegakm1)^2))
        ...
        t0 <- max(sqrt(sum(Xbetakm1^2)), max(sqrt(sum(Omegak^2)), ynorm))
        cond1 <- 1*(r1 < sqrt(n)*tol.abs + tol.rel*t0)
        cond2 <- 1*(s1 < sqrt(p)*tol.abs + 
            tol.rel*sqrt(sum(crossprod(X, Gammak)^2)))
...
MSRL.coef <- function (fit, lambda = NULL) {
    ...
    q <- length(fit$Y.mean)
    if (!fit$standardize) {
        beta.vec <- fit$beta[,lam.ind,drop=FALSE]
        beta.mat <- matrix(beta.vec, byrow=FALSE, nrow=p, ncol=q)
    } else {
        beta.vec <- fit$beta[,lam.ind,drop=FALSE]
        beta.mat <- tcrossprod(1/fit$X.sd, rep(1, q))*matrix(beta.vec, byrow=FALSE, nrow=p, ncol=q)
...
MSRL.cv.omega <- function (X, Y, nlambda, lambda.vec = NULL,
        weighted = FALSE, standardize = FALSE, ADMM = FALSE, nfolds = NULL, 
        delta = .01, tol = 1e-8, quiet = TRUE, inner.quiet=TRUE) {
    ...
    sparsity.mat <- rep(0, length(lambda.vec))
    beta.old <- matrix(0, nrow=p, ncol=q)
    omega.full <- array(0, dim = c(n, q, length(lambda.vec)))
    Gamma <- y
    temp <- svd(y)
    Omega <- tcrossprod(temp$u, temp$v)
    xtxeig <- max(eigen(xtx)$val)
