...
\begin{itemize}
 \item[] \textbf{A1.} The $n \times q$ error matrix $\mathcal{E}$ has $q$ nonzero singular values almost surely. 
...
\State Decompose $(U,D,V) = {\rm svd}(Y + \rho^{-1}\Gamma^{(k)} - X\beta^{(k)})$. 
...
\State Decompose $(\tilde{U}, \tilde{D}, \tilde{V}) = {\rm svd}(Y - X \Gamma^{(k)})$.
	...
	\State Decompose $(\tilde{U}, \tilde{D}, \tilde{V}) = {\rm svd}(Y - X\tilde{\beta})$. 
...
\end{equation} 
From \citet{watson1992characterization}, letting $(U_B, D_B, V_B) = {\rm svd}(B)$, we have 
