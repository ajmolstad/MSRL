...
where $(U,D,V) = {\rm svd}(Y - X\hat{\beta})$. This argument may be useful if convergence is slow with default settings. We decrease the convergence tolerance to display more output for the sake of example. 
...
mod3 <- MSRL.cv(X = X, Y = Y, nlambda = 50, standardize = FALSE, ADMM = FALSE, 
                nfolds = 5,  weighted = FALSE, delta = .1, tol = 1e-8, quiet = TRUE, 
                inner.quiet=TRUE)
```
...
- $\texttt{tol}$ is the convergence tolerance. If the default is not performing well, we recommend model fitting with $\texttt{inner.quiet = FALSE}$ to track progress and adjusting $\texttt{tol}$ as necessary.
